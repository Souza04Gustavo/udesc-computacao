...
-- BLOCO DE PRECEDÊNCIA E ASSOCIATIVIDADE COMPLETO
-- (Menor precedência no topo, maior na base)
%left '||'                       -- Menor precedência
%left '&&'                       -- Precedência maior que '||'
%nonassoc '==' '/=' '<' '>' '<=' '>=' -- Operadores relacionais
%left '+' '-'
%left '*' '/' '%'
%right '!' NOT_UNARIO              -- Maior precedência (operadores unários)
...
Comando  : CmdSe                           { $1 }
         ...
         | ChamadaProc                     { $1 }
         | Retorno                         { $1 }
         -- NOVAS REGRAS DE DECLARAÇÃO COMO COMANDOS:
         | Tipo ListaID ';'                { Decl (map (\id -> id :#: ($1, 0)) $2) }
         | Tipo ID '=' Expr ';'            { DeclComAtrib ($2 :#: ($1, 0)) $4 }
