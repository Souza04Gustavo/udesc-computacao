...
data Expr = Add Expr Expr | Sub Expr Expr | Mul Expr Expr | Div Expr Expr | Neg Expr | IdVar String | Chamada Id [Expr] | Lit String | Const TCons | BConst Bool| Mod Expr Expr | IntDouble Expr | DoubleInt Expr deriving Show
...
data Comando = If ExprL Bloco Bloco | While ExprL Bloco | Leitura Id | Imp Expr | Ret (Maybe Expr) | Proc Id [Expr] | Atrib String Expr | DeclComAtrib Var Expr | Decl [Var] deriving Show
