...
genExpr env@(_, tab) (Ternario cond eTrue eFalse) = do
    lTrue  <- novoLabel
    lFalse <- novoLabel
    lFim   <- novoLabel

    -- 1. Gera o código da condição com os saltos.
    codeCond <- genExprL env cond lTrue lFalse

    -- 2. Gera o código para as duas expressões de resultado.
    (tipoTrue,  codeTrue)  <- genExpr env eTrue
    (tipoFalse, codeFalse) <- genExpr env eFalse
    
    -- A análise semântica já garantiu que os tipos são compatíveis e
    -- inseriu nós de coerção. O tipo final é o tipo da expressão 'true'.
    let tipoFinal = tipoTrue

    -- 3. Monta o bloco de código completo.
    let code = codeCond ++
               lTrue  ++ ":\n" ++
               codeTrue ++
               "\tgoto " ++ lFim ++ "\n" ++
...
genCmd env (DoWhile blocoLoop cond) = do
    -- Precisamos de um único label para marcar o início do corpo.
    lCorpo <- novoLabel
    
    -- Gera o código para o corpo do loop.
    codeBloco <- genBloco env blocoLoop
    
    -- Gera o código para o teste da condição.
    -- Se a condição for VERDADEIRA, saltamos de volta para lCorpo.
    -- Se for FALSA, precisamos de um lugar para "cair". O genExprL exige um lFalse.
    -- Podemos criar um lFim, mas o goto para ele é redundante. Uma otimização
    -- seria ter uma versão de genExprL que não gera o 'goto lFalse'.
    -- Por simplicidade, vamos usar a estrutura padrão.
    lFim <- novoLabel
    codeCond <- genExprL env cond lCorpo lFim
    
    -- Monta a estrutura final.
    return (lCorpo ++ ":\n" ++    -- 1. Marca o início do corpo.
            codeBloco ++          -- 2. Executa o corpo.
            codeCond ++           -- 3. Executa o teste no final.
            lFim ++ ":\n")        -- 4. Marca o fim do loop.
